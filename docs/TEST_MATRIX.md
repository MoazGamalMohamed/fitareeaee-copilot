...
| Published APK install | PASS (emulator) | Exact downloaded v1.0.14 asset fresh-installed after removing only prior emulator app data. Version `1.0.14` / code `20260728`, minimum API 24, and target API 36 cold-launched to Login/Welcome with no matching `FirebaseFailure`, `PERMISSION_DENIED`, fatal exception, or `E/flutter` log. The byte-identical local artifact passed authenticated rider/driver switching and lifecycle smoke; physical-phone v1.0.14 install remains pending. |
