...
final createBookingProvider = FutureProvider.family<BookingModel, BookingModel>((ref, booking) async {
  print('🎬 START: Creating booking for passenger=${booking.passengerId}, driver=${booking.driverId}, trip=${booking.tripId}');
  ...
  try {
    ...
    print('⚠️ Booking data: passengerId=${booking.passengerId}, driverId=${booking.driverId}');
    rethrow;
  }
});
  ...
  return _firestore
      ...
      .map((snapshot) {
        ...
        final bookings = snapshot.docs
            .map((doc) {
              final data = doc.data();
              print('  - Booking ${doc.id}: status=${data['status']}, paymentStatus=${data['paymentStatus']}');
