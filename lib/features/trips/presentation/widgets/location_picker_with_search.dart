    ...
    try {
      ...
      if (predictions.isNotEmpty) {
        ...
        if (mounted && input.length >= 3) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No locations found for "$input". Try a different search.'),
      ...
      setState(() {
        _isSearching = false;
        _predictions = [];
        _showPredictions = !kIsWeb; // Keep visible on mobile for "My Current Location"
      });
    }
  }
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search for a place (e.g., "Dubai Mall", "Cairo Tower")',
                    helperText: 'Type to see suggestions, then tap one to select',
                    helperMaxLines: 2,
                ...
                if (_showPredictions && (_predictions.isNotEmpty || !kIsWeb))
                  Builder(
                    builder: (context) {
                      debugPrint('🎨 Rendering dropdown: showPredictions=$_showPredictions, predictions=${_predictions.length}, isWeb=$kIsWeb');
                      return Container(
                        margin: const EdgeInsets.only(top: 8),
                        decoration: BoxDecoration(
                          color: Colors.white,
                          border: Border.all(color: Colors.blue, width: 2), // Debug: visible border
                          borderRadius: BorderRadius.circular(12),
                          boxShadow: [
                            BoxShadow(
                        ...
                        constraints: const BoxConstraints(minHeight: 50, maxHeight: 250), // Debug: min height
                        child: ListView(
                      shrinkWrap: true,
                      ...
                      children: [
                        ...
                        if (!kIsWeb && _predictions.isNotEmpty) const Divider(height: 8),
                        ...
                        markers: _selectedLocation != null
                            ? {
                                Marker(
                                  ...
                                  draggable: false,  // Disable dragging - force autocomplete
                                ),
                              }
                            : {},
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  backgroundColor: _selectedLocation != null ? null : Colors.grey[300],
                ),
                child: Text(
                  _selectedLocation != null ? 'Confirm Location' : 'Select a location from suggestions',
